...
def data_collator(features: list) -> dict:
    ...
    for ids_l, feature in sorted(zip(len_ids, features), key=lambda x: -x[0]):
        ...
        # edit begin
        # print('\ninput:', tokenizer.convert_ids_to_tokens(ids))
        # print_ids = [tmp_id for tmp_id in labels if tmp_id != 0]
        # print_txt = tokenizer.convert_ids_to_tokens(print_ids)
        # data_cnt += 1
        # print('label:', print_txt)
...
def main():
    ...
    dataset = datasets.load_from_disk("../data/dataset3/dataset_new")
    ...
    training_args = TrainingArguments(
        ...
        output_dir=save_model_dir,  # 保存位置
        logging_steps=100,
        max_steps=2,
        num_train_epochs=1,  # default = 2 * 4,
